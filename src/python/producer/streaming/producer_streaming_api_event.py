...
#sentiment analysis
#Appends a number to the payload to represent the strength of each individual sentiment    
...
#Appends tags based on the celebrity, accessory, event and brand to the payload
...
#Converts a number into type Long
...
class StdOutListener(tweepy.StreamListener):
    def on_status(self, message):
        ...
        try:
	    #Converts the tweet into an avro format and stores it as such
...
if __name__ == '__main__':
	...
	listener = StdOutListener()
        ...
        db = MySQLdb.connect("192.168.*.*","root","","quest_**" )
        cursor = db.cursor()
	...
	track_data = []
        ...
        sql = "select name from quest_streaming.trends where type='event';"
        try:
            cursor.execute(sql)
            results = cursor.fetchall()
            for row in results:
                track_data.append(row[0])
        except Exception,e:
            logging.debug('Problem in event retrieval: %s' % e)
            print 'Problem in event retrieval: ',e
            twitter_utils.sendErrorMail('There was an issue in retrieving the list of events from the database. The error is %s.' % e)
	...
	logging.debug('There was an error in the main loop of the producer. The error is: %s' % e)
	print "Error in main loop"
	print Exception,e
        twitter_utils.sendErrorMail('There was an issue in the main loop of the producer. The error is %s.' % e)
